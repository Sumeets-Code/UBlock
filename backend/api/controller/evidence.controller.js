...
const getAllEvidences = async (req, res) => {
  try {
    ...
    const data = await evidenceService.fetchAllEvidences({
      search,
      category,
      status,
    });
  ...
  try {
    ...
    if (!title || !caseNumber || !collectedBy || !collectionDate) {
      return res
        ...
        .json({
          message:
            "title, caseNumber, collectedBy and collectionDate are required",
        });
    }
...
const statusUpdate = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Status updated", evidence: updated });
  } catch (err) {
...
const getAuditLog = async (req, res) => {
  try {
    const log = await evidenceService.fetchCustodyAuditLog(req.params.id);
...
const handlePrepareUpload = async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({ message: "No file uploaded" });
    }
    ...
    const result = await evidenceService.prepareUpload(req);
...
const handleConfirmUpload = async (req, res) => {
  try {
    const { mongoId, evidenceId, txHash, walletAddress } = req.body;
    ...
    return res.status(200).json({
      message: "Evidence confirmed and secured on blockchain",
      evidence,
    });
  } catch (err) {
