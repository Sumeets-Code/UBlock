...
const getEIP1559Fees = async () => {
  try {
    // eth_feeHistory gives us the last block's base fee
    const feeHistory = await window.ethereum.request({
      method: "eth_feeHistory",
      params: ["0x1", "latest", [10]], // 1 block, 10th percentile tip
    });

    const baseFee = BigInt(feeHistory.baseFeePerGas?.[0] ?? "0x0");
    // Use 10th-percentile tip for minimal cost — 1 gwei is enough on Sepolia
    const tip = BigInt(feeHistory.reward?.[0]?.[0] ?? "0x3B9ACA00"); // 1 gwei default

    // maxFeePerGas = 2× baseFee + tip — the 2× buffer absorbs base fee spikes
    // without overpaying (unspent maxFee is refunded by the EVM)
    ...
    // Fallback to legacy gasPrice if the RPC doesn't support eth_feeHistory
...
export const useBlockchain = () => {
  const { account, isConnected, isCorrectNetwork, switchToSepolia } =
    useWallet();

  const [txHash, setTxHash] = useState(null);
  const [txStatus, setTxStatus] = useState("idle"); // idle | pending | confirming | confirmed | error
  const [txError, setTxError] = useState(null);
  const [gasCost, setGasCost] = useState(null); // human-readable e.g. "0.000023 ETH"
  ...
  const registerEvidenceOnChain = useCallback(
    async (ipfsHash32, mongoId, fileType) => {
      setTxError(null);
      setTxHash(null);
      setTxStatus("idle");
      setGasCost(null);
      ...
      try {
        ...
        const calldata = IFACE.encodeFunctionData('registerEvidenceByUser', [
          ipfsHash32,   // bytes32 — ethers handles the 0x prefix correctly
          mongoId,      // string
          fileType,     // string
        ]);
        ...
        try {
          ...
          gasLimit = "0x" + Math.ceil(parseInt(est, 16) * 1.1).toString(16);
          // const gasLimit = '0x' + (BigInt(est) * 120n / 100n).toString(16);
        } catch {
          gasLimit = "0x" + (95_000).toString(16); // safe fallback
        }

        // Get EIP-1559 fee params — cheapest possible gas
        const fees = await getEIP1559Fees();

        // Show estimated cost to user before they sign
        if (fees.maxFeePerGas) {
          const estimatedWei = BigInt(gasLimit) * BigInt(fees.maxFeePerGas);
          const ethStr = (Number(estimatedWei) / 1e18).toFixed(6);
          setGasCost(`~${ethStr} ETH (max, unused gas is refunded)`);
        }
        ...
        if (receipt.status !== "0x1") {
          throw new Error(
            "Transaction reverted on-chain. Check contract address and inputs.",
          );
        }
        ...
        if (receipt.effectiveGasPrice) {
          const actualWei = BigInt(receipt.gasUsed) * BigInt(receipt.effectiveGasPrice);
          setGasCost(`${(Number(actualWei) / 1e18).toFixed(6)} ETH (actual)`);
        }
  ...
  return { registerEvidenceOnChain, txHash, txStatus, txError, gasCost, reset };
};
