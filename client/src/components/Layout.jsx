...
function Layout({ page, setPage, children }) {
  ...
  const pageLabels = { dashboard: "Command Dashboard", upload: "Upload Evidence", evidence: "Evidence Vault", reports: "Generate Reports" };

  return (
    <div className="app-layout">
      {sidebarOpen && <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 99 }} onClick={() => setSidebarOpen(false)} />}
      <aside className={`sidebar${sidebarOpen ? " open" : ""}`}>
        ...
        <nav className="sidebar-nav">
          ...
          {NAV.map(item => (
            <div key={item.path} className={`nav-item${page === item.path ? " active" : ""}`} onClick={() => { setPage(item.path); setSidebarOpen(false); }}>
